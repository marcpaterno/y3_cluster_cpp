...
using y3_cluster::SampleVariance_t, y3_cluster::IntegrationRange,
  y3_cluster::OMEGA_Z_SDSS, y3_cluster::bessel_polynomial_integral;

void
write_matter_power(
  std::shared_ptr<y3_cluster::Interp2D const>& matter_power_lin)
{
  ...
  for (auto z : zs) {
    for (auto i = 0; i < 101; i++) {
      ...
      output_file << z << "," << k << "," << p_k << "," << p_k_k_cubed << "\n";
    }
  }
  output_file.close();
}
...
write_bessel_sums(const y3_cluster::SampleVariance_t& sv)
{
  ...
  for (auto i = 0; i < 1001; i++) {
    ...
    output_file << k << "," << sv.compute_sum_over_bessels(k, 0, 0) << ','
...
main()
{
  ...
  std::vector<double> ks{
    {0.0001, 0.0005, 0.001, 0.002, 0.003, 0.004, 0.005, 0.01, 0.05}};
  std::vector<std::pair<double, double>> zbins{
    ...
    for (auto k : ks) {
      for (auto l = 0u; l < 8; l++) {
        ...
        const double int_value =
          bessel_polynomial_integral(2, l, k, rmin, rmax);
        const double normalized [[maybe_unused]] =
          int_value * 3 / (rmax * rmax * rmax - rmin * rmin * rmin);

        std::cout << zlow << ',' << zhigh << ',' << k << ',' << l << ','
  ...
  std::cout << "DCom({0.1, 0.3}) = {" << 1.1 * da_f->eval(0.1) * h << ", "
            << 1.3 * da_f->eval(0.3) * h << "}\n";

  OMEGA_Z_SDSS omega_z;
  std::vector<y3_cluster::IntegrationRange> zs{
    {0.1, 0.3}, {0.3, 0.5}, {0.5, 0.7}};
  SampleVariance_t sv(omega_z, zs, h);
  write_bessel_sums(sv);

  std::shared_ptr<const y3_cluster::Interp2D> matter_power_lin =
    std::make_shared<const y3_cluster::Interp2D>(
  ...
  for (auto l = 0u; l < 10; l++)
    std::cout << "kay(" << l << ", " << 0.1
              << ") = " << y3_cluster::survey_mask_kay(omega_z, l, 0.1) << '\n';

  std::cout << "About to integrate...\n";

  ProfilerStart("/cosmosis/cosmosis-standard-library/y3_cluster_cpp/test/"
                "sample_variance_output/dump.txt");
