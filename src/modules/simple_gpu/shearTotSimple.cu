// Total tangential shear (1h + b*2h) GPU module WITHOUT selection function.
//
// This module computes the bin-averaged tangential shear:
//   <gamma_t>(R) = N^{-1} * int dlnM int dz  Omega(z) * (dV/dOmega/dz)(z)
//                  * n(M,z) * P(lo|M,z) * K(zo|z) * gamma_t(R, M, z)
//
// where gamma_t = DSigma_tot(R, M, z) * Sigma_crit_inv(z)
// and DSigma_tot = DSigma_1h(R,M) + b(M,z) * DSigma_2h(R,z)
//
...
// shearTotSimple: Total tangential shear without selection function.
// Integrates over (lo, zt, lnM) with MOR and gamma_t(R, M, z).
class shearTotSimple {
public:
...
private:
  ...
  std::optional<y3_cuda::GAMMA_MAX> gamma_;
...
public:
  explicit shearTotSimple(cosmosis::DataBlock& cfg)
  {
    ...
    if (rc != DBS_SUCCESS) {
      throw std::runtime_error(
        "shearTotSimple: failed to read do_cartesian_product_of_bins\n");
    }
  }
  ...
  operator()(double lo, double zt, double lnM) const
  {
    ...
    // Total tangential shear: DSigma_tot * Sigma_crit_inv
    double const gamma_v = (*gamma_)(radius_, lnM, zt);

    return common * mor_v * photoz * gamma_v;
  }

  static char const* module_label() { return "shearTotSimple"; }
  ...
  make_integration_volumes(cosmosis::DataBlock& cfg)
  {
    return y3_cuda::make_integration_volumes_wall_of_numbers(
      cfg, shearTotSimple::module_label(), "lo", "zt", "lnm");
  }

  static grid_t make_grid_points(cosmosis::DataBlock& cfg)
  {
    return y3_cluster::make_grid_points_wall_of_numbers(
      cfg, shearTotSimple::module_label(), "zo_low", "zo_high", "radii");
  }
};

DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(shearTotSimple)
