...
public:
  ...
  explicit avgDSigmaMiscBu(cosmosis::DataBlock& config);
...
avgDSigmaMiscBu::avgDSigmaMiscBu( DataBlock& cfg)
{
  auto rc = 
...
void
avgDSigmaMiscBu::set_sample(DataBlock& sample)
{
...
void
avgDSigmaMiscBu::set_grid_point(
...
avgDSigmaMiscBu::operator()(   double lo,
                              double zt,
                              double lnM) const
{
  ...
  auto const val = (*sigma_mis)(radius_, lnM, zt) *
...
std::vector<avgDSigmaMiscBu::volume_t>
avgDSigmaMiscBu::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgDSigmaMiscBu::module_label(), "lo", "zt", "lnm");
}

avgDSigmaMiscBu::grid_t
avgDSigmaMiscBu::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgDSigmaMiscBu)
