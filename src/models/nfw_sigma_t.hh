...
class nfw_sigma{
public:
  explicit nfw_sigma(double c, double om) 
    : _c(c), _om(om)
  {}
  ...
  operator()(double r, double M) const /* M in h^-1 M_solar, M_{200} here */
  {
    ...
    if (r_ratio < 1.0)
    {  
        res= 2.0 * coeff / (r_ratio * r_ratio - 1) * (1- 2.0/std::sqrt(1-r_ratio*r_ratio) * std::atanh(std::sqrt( (1.-r_ratio)/(1.+r_ratio) )) ) ;
    } else if (r_ratio == 1.0){
        res = 2.0 * coeff / 3.0;
    } else {
        res= 2.0 * coeff / (r_ratio * r_ratio - 1) * (1- 2.0/std::sqrt(r_ratio*r_ratio -1) * std::atan(std::sqrt( (r_ratio -1)/(1.+r_ratio) )) ) ;
    }
    return res; 
    /*if(r_ratio<1.){
