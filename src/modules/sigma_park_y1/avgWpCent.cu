...
public:
  ...
  explicit avgWpCent(cosmosis::DataBlock& config);
...
avgWpCent::avgWpCent( DataBlock& cfg)
{
  auto rc = 
...
void
avgWpCent::set_sample(DataBlock& sample)
{
...
std::vector<avgWpCent::volume_t>
avgWpCent::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgWpCent::module_label(), "lo", "lt", "zt", "lnm");
}

avgWpCent::grid_t
avgWpCent::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgWpCent)
