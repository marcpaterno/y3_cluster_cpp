...
public:
	     ...
	     double scaled_theta) const
  {
    ...
    double const N = jacob_N * omega_z_v * dv_do_dz_v * zo_zt_v * hmf_v * mor_v * ( lo_lt_v*fcen_/lc_jacob + lc_lt_v*(1.0-fcen_)*roffset(R)*lo_lc(lo, lc, R)); //99. = lc_ir_.jacobian()
    double const Nw = N * w;//Why times jacob again?

    auto const  gamma_t_int =jacob * omega_z_v * dv_do_dz_v * zo_zt_v * hmf_v * mor_v * w ;
    auto gamma_t_cen = [this, lnM, zt, A, lc_jacob, lo_lt_v](double radius){ return fcen_ * del_sig_cen(radius, lnM, zt) * exp(A * T_cen(radius, lnM))*lo_lt_v/(lc_jacob*6.28318530718); };
    auto gamma_t_mis = [this, lnM, zt, A, R, lo, lc, theta, lc_lt_v](double radius){ return (1.0 - fcen_) * roffset(R)*lo_lc(lo, lc, R) * del_sig_cen(std::sqrt(radius*radius + R*R + 2*R*radius * std::cos(theta)), lnM, zt) *  exp(A * T_cen(radius, lnM))*lc_lt_v/(6.28318530718) ; } ;
