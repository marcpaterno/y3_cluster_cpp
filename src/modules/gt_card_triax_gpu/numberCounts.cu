...
#include "models/mor_des_triax.cuh"
...
// numberCountsCardTriax is a class that models the concept of
...
private:
  ...
  std::optional<y3_cuda::MOR_DES_TRIAX> mor_triax;
  std::optional<y3_cuda::INT_LC_LT_DES_t> lc_lt;
  std::optional<y3_cuda::INT_ZO_ZT_DES_t> int_zo_zt;
  std::optional<y3_cuda::TRIAX_RICHNESS> lnA_cosi; 
...
public:
  ...
  {
    size_t dev_size = 0;
    //if (mor_triax) dev_size += (*mor_triax).get_device_mem_footprint();
  ...
  explicit numberCountsCardTriax(cosmosis::DataBlock& config);
  ...
  // object.
  __host__ __device__ double operator()(double lo, double lt, double zt, double lnM, double mu) const;
...
numberCountsCardTriax::numberCountsCardTriax(DataBlock& cfg)
{
  auto rc = 
  ...
  if (rc != DBS_SUCCESS) {
    throw std::runtime_error("numberCountsCardTriax failed to find do_cartesian_product_of_bins\n");
  }
}

void
numberCountsCardTriax::set_sample(DataBlock& sample)
{
  ...
  hmf.emplace(sample);
  mor_triax.emplace(sample);
  lc_lt.emplace(sample);
}

void
numberCountsCardTriax::set_grid_point(grid_point_t grid_point)
{
...
numberCountsCardTriax::operator()(double lo,
                         double lt,
                         double zt,
                         double lnM,
                         double mu) const
{
  ...
  double const lc = lo;
  double const mor_v = (*mor_triax)(lo, lnM, zt, mu);
...
char const*
numberCountsCardTriax::module_label()
{
  return "numberCountsCardTriax";
}
...
std::vector<numberCountsCardTriax::volume_t>
numberCountsCardTriax::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
...
numberCountsCardTriax::grid_t
numberCountsCardTriax::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(numberCountsCardTriax)
