...
public:
  ...
  explicit avgWpCentY1(cosmosis::DataBlock& config);
...
avgWpCentY1::avgWpCentY1( DataBlock& cfg)
{
  auto rc = 
...
void
avgWpCentY1::set_sample(DataBlock& sample)
{
...
avgWpCentY1::operator()(double lo,
                      double lt,
                      double zt,
                      double lnM) const
{
  ...
  auto const val = (*wp_cc)(radius_, lnM, zt) *
...
std::vector<avgWpCentY1::volume_t>
avgWpCentY1::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgWpCentY1::module_label(), "lo", "lt", "zt", "lnm");
}

avgWpCentY1::grid_t
avgWpCentY1::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgWpCentY1)
