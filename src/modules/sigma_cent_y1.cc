...
// sigma_cent_y1 is a class that models the concept of
...
public:
  ...
  explicit sigma_cent_y1(cosmosis::DataBlock& config);
...
sigma_cent_y1::sigma_cent_y1(DataBlock& config)
  : lc_lt()
  , mor()
  , omega_z()
  , dv_do_dz()
  ...
  , sigma()
  , zo_low_(
      config.view<std::vector<double>>(sigma_cent_y1::module_label(), "zo_low"))
  , zo_high_(config.view<std::vector<double>>(sigma_cent_y1::module_label(),
                                              "zo_high"))
  , radii_(
      config.view<std::vector<double>>(sigma_cent_y1::module_label(), "radii"))
{}

void
sigma_cent_y1::set_sample(DataBlock& sample)
{
...
std::vector<double>
sigma_cent_y1::operator()(double lo, double lt, double zt, double lnM) const
{
...
char const*
sigma_cent_y1::module_label()
{
  return "sigma_cent_y1";
}
...
std::vector<sigma_cent_y1::volume_t>
sigma_cent_y1::make_integration_volumes(cosmosis::DataBlock& cfg)
{
  try {
