...
TEST_CASE("MOR_Y3xSPT_t works")
{
    ...
    auto lnm200m_in = read_vector("mass_conversion/lnm200m_z0.5_child18_msunhinv_mor_y3xspt_t.txt");
    auto lnm500c_in = read_vector("mass_conversion/lnm500c_z0.5_child18_msunhinv_mor_y3xspt_t.txt");
    auto mconv = std::make_shared<Interp1D const>(lnm200m_in, lnm500c_in);

    // Set up the MOR object
    MOR_Y3xSPT_t mor = MOR_Y3xSPT_t(Mmin, M1, alpha, epsilon, sigintr, Asz, Bsz, Csz, siglnzeta,
                                    r, zplam, lnMpsz, zpsz, omega_m, omega_l, omega_k, mconv);
