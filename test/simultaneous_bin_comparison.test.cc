...
TEST_CASE(
  "Check that integrating multiple bins simultaneously does not change answers")
{
  ...
  long long maxeval = 999999999;
  double sigma_intr = 0.15; // this is a parameter that should come from
                            // cosmosis
  double alpha = 0.65; // this is a parameter that should come from cosmosis
  y3_cluster::MOR_t mor{
    y3_cluster::mz_power_law{9.1e-9, alpha, 0.0}, sigma_intr, alpha};
  ...
  y3_cluster::DV_DO_DZ_t dvdodz(
    da_f, y3_cluster::EZ(Omega_M, Omega_L, Omega_K), h);
