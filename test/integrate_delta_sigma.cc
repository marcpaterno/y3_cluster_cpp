...
struct delta_sigma_parameters {
  ...
  delta_sigma_parameters(double lo, double lt, double zt, double R, double lnM)
    : lo(lo), lt(lt), zt(zt), R(R), lnM(lnM)
  {}
};

using y3_cluster::LO_LC_t, y3_cluster::LC_LT_t, y3_cluster::ROFFSET_t,
  y3_cluster::T_CEN_t, y3_cluster::DEL_SIG_y1, y3_cluster::IntegrationRange;
...
integrate_del_sig_cen(Integrator I,
                      ...
                      double epsabs = 1e-12)
{
  const auto &lo = params.lo, lt = params.lt, zt = params.zt, R = params.R,
             lnM = params.lnM;
  ...
  return I.integrate(
    [&](double scaled_A) {
      ...
      return lo_lt_v * exp(A * tcen(R, lnM)) * dsc(R, lnM, zt);
    },
    epsrel,
    epsabs);
}
  ...
  return I.integrate(
    [&](double scaled_R_mis,
      ...
      const double R_mis = R_mis_range.transform(scaled_R_mis),
                   ...
                   A = A_range.transform(scaled_A), lc_lt_v = lc_lt(lc, lt, zt),
                   R_adj =
                     sqrt(R * R + R_mis * R_mis + 2 * R * R_mis * cos(theta));
      return lc_lt_v * lo_lc(lo, lc, R_mis) * roffset(R_mis) *
             exp(A * tcen(R_adj, lnM)) * dsc(R_adj, lnM, zt);
    },
    epsrel,
    epsabs);
}
...
main()
{
  ...
  delta_sigma_parameters dsp(25, 25, 0.2, 0.5, std::log(1e15));

  // Parameter exploration ranges
  IntegrationRange lo_ir{20, 28}, lt_ir{2, 50}, zt_ir{0.1, 0.3}, R_ir{0., 1.0},
    lnM_ir{std::log(5e11), std::log(1e16)};

  // Proper integration ranges
  IntegrationRange A_ir{-0.01, 0.01}, R_i{0.1, 1}, theta_ir{0.0, 6.28318530718},
    lc_ir{2, 50};
  ...
  do {
    auto res_cen = integrate_del_sig_cen(c, dsp, lc_lt, tcen, dsc, A_ir);
    auto res_mis = integrate_del_sig_mis(
      c, dsp, lc_lt, lo_lc, roffset, tcen, dsc, R_ir, theta_ir, lc_ir, A_ir);
    std::cout << dsp.lo << "," << dsp.lt << "," << dsp.zt << "," << dsp.R << ","
              << dsp.lnM << "," << res_cen.value << "," << res_cen.status << ","
              << res_cen.error << "," << res_cen.prob << "," << res_mis.value
              << "," << res_mis.status << "," << res_mis.error << ","
              << res_mis.prob << '\n';
  } while (pse.next());
