...
private:
  ...
  // and the Kappa profile
  std::optional<y3_cuda::KAPPA_MAX> kappa;
...
public:
  ...
  explicit avgKappaCentBu(cosmosis::DataBlock& config);
  ...
  get_device_mem_footprint()
  {
    ...
    if ((bool)kappa == true) dev_size += (*kappa).get_device_mem_footprint();
    return dev_size;
  }
...
avgKappaCentBu::avgKappaCentBu( DataBlock& cfg)
{
  auto rc = 
...
void
avgKappaCentBu::set_sample(DataBlock& sample)
{
  ...
  hmf.emplace(sample);
  mor.emplace(sample);
  kappa.emplace(sample);
}

void
avgKappaCentBu::set_grid_point(
...
avgKappaCentBu::operator()(   double lo,
                              double zt,
                              double lnM) const
{
...
std::vector<avgKappaCentBu::volume_t>
avgKappaCentBu::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgKappaCentBu::module_label(), "lo", "zt", "lnm");
}

avgKappaCentBu::grid_t
avgKappaCentBu::make_grid_points(cosmosis::DataBlock& cfg)
{
