                         ...
                         bool test = true)
  {
    ...
    for (auto i = 0u; i < top; i++) {
      ...
      auto print_line = [print, bin_cen](const char* cen_or_mis,
                                         const auto bin,
                                         const auto bin_single,
                                         const auto res,
                                         const auto res_single) {
        if (print)
          std::cout << bin_cen.lo_ir.transform(0.0) << ", "
                    << bin_cen.lo_ir.transform(1.0) << ", "
                    << bin_cen.zo_ir.transform(0.0) << ", "
                    << bin_cen.zo_ir.transform(1.0) << ", " << cen_or_mis << ","
                    << bin_single.N << ", " << bin.N << ", "
                    << res_single.all_converged() << ", " << res.all_converged()
                    << ", " << bin_single.N_error << ", " << bin.N_error << ", "
                    << bin_single.N_prob << ", " << bin.N_prob << '\n';
      };
