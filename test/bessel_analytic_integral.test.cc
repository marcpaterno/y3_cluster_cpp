...
using y3_cluster::bessel_polynomial_integral,
  ...
  y3_cluster::sinusoid_polynomial_integral, y3_cluster::integer_pow;

TEST_CASE("Test analytic sin/cos polynomial integrals")
{
  cubacpp::QAG qag;

  const auto test_integral = qag.with_range(1, 2).integrate(
    [](double x) { return x * gsl_sf_bessel_jl(3, x); }, 1e-5, 1e-18);

  REQUIRE(test_integral.status == 0);

  const auto test_analytic = [](double x) {
    return -1 * (3 * sinusoid_polynomial_integral(-3, 0.5, x).first +
                 (3 / x) * gsl_sf_bessel_jl(0, x) + 3 * gsl_sf_bessel_jl(1, x) +
    ...
    for (auto const& [minx, maxx] : ranges) {
      for (auto k : ks) {
        ...
        for (auto l = 0u; l < maxl; l++) {
          ...
          CHECK(bessel_integral.value ==
                Approx(list[l]).epsilon(1e-3).margin(1e-10));
        }
      }
    }
  }
