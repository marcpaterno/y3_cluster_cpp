...
namespace y3_cluster {
   ...
   *
   * The bin is specified by `lo_ir` and `zo_ir` for richness, redshift,
   * respectively. Integrated values are in `gamma_ts`, `N`, `Nw`, `Nb`, with
   * error and probability values for each in `*_error[s]` and `*_prob[s]`.
   */
  struct Gamma_T_Integrated_Bin_Result {
    ...
    double N, N_error, N_prob, Nw, Nw_error, Nw_prob, Nb, Nb_error, Nb_prob;
    ...
    Gamma_T_Integrated_Bin_Result(std::size_t which_richness,
      ...
      , gamma_t_probs()
    {
      auto const nradii = radius.size(), npzsource = gt.npzsource;

      // The location of these bin results, as an offset in the integration
      // output
   ...
   *
   * The integration bins all share the `neval`, `nregions`, and `status`
   * parameters from the integration algorithm (see `cubacpp` for details).
   *
   * Results for each specific bin can be accessed from
   * `Gamma_T_Integrated_Bin_Result_S` as in a vector.
   */
  struct Gamma_T_Integrated_Bin_Result_S
    ...
    Gamma_T_Integrated_Bin_Result_S(std::size_t i,
                                    std::size_t e,
                                    std::vector<long long> neval,
                                    std::vector<int> nregions,
                                    std::vector<int> status)
  ...
  operator<<(std::ostream& os, Gamma_T_Integrated_Bin_Result_S const& res)
  {
    for (auto i = 0u; i < res.neval.size(); i++) {
      os << "redshift bin " << i << ": neval: " << res.neval[i]
         << " nregions: " << res.nregions[i] << " status: " << res.status[i]
         << '\n';
    }
    for (auto const& bin : res) {
      // Print out bin info
      os << "Bin: [zmin, zmax] = [" << bin.zo_ir.transform(0.0) << ", "
         << bin.zo_ir.transform(1.0) << "], [lomin, lomax] = ["
         << bin.lo_ir.transform(0.0) << ", " << bin.lo_ir.transform(1.0)
         << "]\n";

      // Print out number counts
      os << "N:  " << bin.N << " +/- " << bin.N_error << " prob: " << bin.N_prob
         << '\n';
      ...
      for (auto i = 0u; i < bin.gamma_ts.size(); i++)
        for (auto j = 0u; j < bin.gamma_ts[0].size(); j++)
          os << "gamma_t (src bin " << i << ", R = " << bin.radius[j]
             << "): " << bin.gamma_ts[i][j] << " +/- "
             << bin.gamma_t_errors[i][j] << " prob: " << bin.gamma_t_probs[i][j]
             << '\n';
    }
  ...
  namespace {
    ...
    {
      Gamma_T_Integrated_Bin_Result_S return_arr(n_richness,
                                                 n_redshift,
                                                 {results.neval},
                                                 {results.nregions},
                                                 {results.status});
      ...
      for (auto loi = 0u; loi < n_richness; loi++) {
        for (auto zoi = 0u; zoi < n_redshift; zoi++) {
          return_arr[loi * n_redshift + zoi] =
            Gamma_T_Integrated_Bin_Result(loi, zoi, gt, results);
        }
      }
    ...
    make_gamma_t_integrated_bins(
      const std::vector<std::pair<INTEGRAND, cubacpp::integration_results_v>>&
        results,
      const size_t n_richness)
    {
      std::vector<long long> neval;
      ...
      Gamma_T_Integrated_Bin_Result_S return_arr(
        n_richness, results.size(), neval, nregions, status);
      ...
      for (auto zoi = 0u; zoi < n_redshift; zoi++) {
        for (auto loi = 0u; loi < n_richness; loi++) {
          ...
          return_arr[loi * n_redshift + zoi] =
            Gamma_T_Integrated_Bin_Result(loi, 0, igrnd, res);
        }
      }
   ...
   * Template parameters:
   *
   * MODELS - A structure representing the specific models to be used in the
   * integration, see `src/default_models.hh` for an example. Allows easily
   * dropping in your own model, e.g. if you want to test a different
   * mass-observable relationship, just subclass DefaultModels with a different
   * MOR type.
   *
   * NRADII - The number of radii at which to compute gamma_T. Ideally this
   * could be specified at runtime; for now it must be statically known.
   *
  ...
  struct Gamma_T_Integrand {
    ...
    // TODO: Matteo uses {z_ir_[i].min - 3.5 * z_sigma, z_ir_[i].max + 3.5 *
    // z_sigma}
  ...
  public:
    ...
    Gamma_T_Integrand(double fcen,
      ...
      , lnM_ir_(lnM_ir)
      , lo_ir_(lo_ir)
      , lt_ir_(y3_cluster::transform(
          lo_ir,
          std::function([](y3_cluster::IntegrationRange const& ir) {
            return y3_cluster::IntegrationRange{
              0.3 * ir.transform(0.0),
              (2 * ir.transform(1.0) > 150) ? 150 : 2 * ir.transform(1.0)};
          })))
      , lc_ir_(lt_ir_)
      , zo_ir_(zo_ir)
      , zt_ir_(y3_cluster::transform(
          zo_ir,
          std::function([](y3_cluster::IntegrationRange const& ir) {
            const double z_sigma = 0.01;
            return y3_cluster::IntegrationRange{
              ir.transform(0.0) - 3.5 * z_sigma,
    ...
    // Alternatively, can automatically construct each model component given a
    // datablock.
    Gamma_T_Integrand(cosmosis::DataBlock& sample,
      ...
      , zt_ir_(y3_cluster::transform(
          zo_bins,
          std::function([&sample](y3_cluster::IntegrationRange const& ir) {
    ...
    /* Common integrand functionality. Do not call this directly, you can
     * probably ignore it.
     *
    ...
    {
      std::size_t const nrichness = lo_ir_.size(), nredshift = zo_ir_.size(),
                        nradii = r.size();

      auto return_arr =
        IntegrandResult((nradii * npzsource + 3) * nrichness * nredshift);
      ...
      for (std::size_t zoi = 0; zoi < nredshift; zoi++) {
        ...
        //  gamma_t[i] = (1.0 + m_shear) / sig_crit * gamma_radial_dep(r[i],
        //  zt);
        auto gamma_t = std::vector<double>(nradii * npzsource);
        for (auto srci = 0u; srci < npzsource; srci++) {
          for (auto i = 0u; i < nradii; i++) {
            // Nw intentionally left out - returned in return_arr to be used
            // further on
            double sig_crit_inv = sig_crit_inv_(srci, zt);
            gamma_t[srci * nradii + i] =
              (1.0 + m_shear) * sig_crit_inv * gamma_radial_dep(r[i], zt);
          }
        }

        for (std::size_t loi = 0; loi < nrichness; loi++) {
          auto const richness_bin_start =
            loi * nredshift * (nradii * npzsource + 3);
          ...
          // eq. (28)
          auto redshift_bin_start =
            richness_bin_start + zoi * (nradii * npzsource + 3);

          for (auto srci = 0u; srci < npzsource; srci++) {
            for (auto i = 0u; i < nradii; i++) {
              // Nw intentionally left out - returned in return_arr to be used
              // further on
              return_arr[redshift_bin_start + nradii * srci + i] =
                gamma_t[srci * nradii + i] * gamma_t_int * N_mult;
            }
          }
                ...
                double scaled_theta) const
    {
      ...
      auto jacob_N = [=](std::size_t loi, std::size_t zoi) {
        return lnM_ir_.jacobian() * lo_ir_[loi].jacobian() *
               ...
               zt_ir_[zoi].jacobian() * R_ir_.jacobian();
      };
      auto jacob_G = [=](std::size_t loi, std::size_t zoi) {
        return lnM_ir_.jacobian() * lo_ir_[loi].jacobian() *
               lt_ir_[loi].jacobian() * lc_ir_[loi].jacobian() *
               zt_ir_[zoi].jacobian() * R_ir_.jacobian() * A_ir_.jacobian() *
               theta_ir_.jacobian();
      };
      ...
      auto N_mis = [=](std::size_t loi, std::size_t zoi) {
        ...
        return (1.0 - fcen_) * lo_lc(lo, lc, R) * lc_lt(lc, lt, zt) *
               roffset(R);
      };

      // eq. (30)
      // For the following lambda function, `radius` corresponds to what is
      // called `R` in the paper, and `R` corresponds to what is called
      // `R_{mis}` in the paper eq. (31)
      auto gamma_t_mis = [this, N_mis, A, lnM, R, theta](double angle,
                                                         double zt) {
        // NB: Angle is in arcminutes, must convert to radians
        // TODO: Check this math
        // YZ comment: TODO check unit, this might have been a mpc vs mpc/h
        // thing going on
        double const radius = da_->eval(zt) * angle * pi() / 180.0 / 60.0,
                     adjusted_R = std::sqrt(radius * radius + R * R +
                                            2 * R * radius * std::cos(theta));
        return (1.0 / 6.28318530718) * exp(A * T_cen(adjusted_R, lnM)) /
               A_ir_.jacobian() * del_sig(adjusted_R, lnM, zt);
      };

      return integrand_common(
        scaled_lt, scaled_zt, lnM, jacob_N, jacob_G, N_mis, gamma_t_mis);
    }
             ...
             double scaled_A) const
    {
      ...
      auto jacob_N = [=](std::size_t loi, std::size_t zoi) {
        return lnM_ir_.jacobian() * lo_ir_[loi].jacobian() *
               lt_ir_[loi].jacobian() * zt_ir_[zoi].jacobian();
      };
      ...
      auto gamma_t_cen = [this, N_cen, A, lnM](double angle, double zt) {
        ...
        // TODO: Check this math
        double const radius = da_->eval(zt) * angle * pi() / 180.0 / 60.0;
        return exp(A * T_cen(radius, lnM)) / A_ir_.jacobian() *
               del_sig(radius, lnM, zt);
      };

      return integrand_common(
        scaled_lt, scaled_zt, lnM, jacob_N, jacob_G, N_cen, gamma_t_cen);
    }

    /* Integrates the _centered_ component of gamma_t, N expressions, and
     * returns the raw results of the integrand. Same arguments as
     * `integrate_centered`, `integrate_miscentered`.
     */
    ...
    {
      return i.integrate(
        [this](double scaled_lo,
               double scaled_lt,
               double scaled_zt,
               double scaled_lnM,
               double scaled_A) {
          return centered(
            scaled_lo, scaled_lt, scaled_zt, scaled_lnM, scaled_A);
        },
        epsrel,
        epsabs);
    }
    ...
    /* Integrates the _centered_ component of the gamma_T, N expressions, and
     * returns a pair of (results, bins), where `results` is the raw `cubacpp`
     * output, and `bins` is an array of `Gamma_T_Integrated_Bin_Result`
     *
     * Arguments:
     *
    ...
    {
      std::vector<
        std::pair<Gamma_T_Integrand<MODELS>, cubacpp::integration_results_v>>
        results;
      for (const auto zir : zo_ir_) {
        Gamma_T_Integrand<MODELS> igrnd = (*this).with_bins(lo_ir_, {{zir}});
        results.push_back(
          {igrnd, igrnd.raw_integrate_centered(i, epsrel, epsabs)});
      }
      return make_gamma_t_integrated_bins(results, lo_ir_.size());
    }

    /* Integrates the _mis_centered component of the gamma_T, N expressions, and
     * returns a pair of (results, bins), where `results` is the raw `cubacpp`
     * output, and `bins` is an array of `Gamma_T_Integrated_Bin_Result
     *
     * Arguments:
    ...
    integrate_miscentered(Integrator i, double epsrel, double epsabs)
    {
      ...
      for (const auto zir : zo_ir_) {
        Gamma_T_Integrand<MODELS> igrnd = (*this).with_bins(lo_ir_, {{zir}});
        results.push_back(
          {igrnd, igrnd.raw_integrate_miscentered(i, epsrel, epsabs)});
      }
      return make_gamma_t_integrated_bins(results, lo_ir_.size());
    }
  };
                         ...
                         std::size_t n_radii)
  {
    ...
    auto const z_da = read_vector("z.txt"), da_arr = read_vector("d_a.txt");

    auto da_f = std::make_shared<y3_cluster::Interp1D const>(z_da, da_arr);

    y3_cluster::weighted_sigma_crit_inv sci({}, z_da, da_arr, {}, {});

    return {fcen,    mor,   lo_lc,    lc_lt,  zo_zt, roffset, t_cen,
            t_mis,   a_cen, a_mis,    hmb,    hmf,   del_sig, dv_do_dz,
            omega_z, sci,   da_f,     lnM_ir, lo_ir, zo_ir,   zs_ir,
            R_ir,    A_ir,  theta_ir, rarray};
  }
