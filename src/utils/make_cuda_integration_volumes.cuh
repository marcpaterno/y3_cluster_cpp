#ifndef Y3_CLUSTER_CPP_MAKE_CUDA_INTEGRATION_VOLUMES_HH
#define Y3_CLUSTER_CPP_MAKE_CUDA_INTEGRATION_VOLUMES_HH
...
namespace y3_cluster {
  ...
  template <typename... Ts>
  std::vector<quad::Volume<double, sizeof...(Ts)>>
  make_cuda_integration_volumes_cartesian_product(cosmosis::DataBlock& cfg,
                                             std::string const& modulelabel,
                                             Ts... names);

  namespace detail {
    template <std::size_t N>
    using integration_boundary = std::array<double, N>;
    template <typename T>
      ...
      std::index_sequence<Is...> /* unused */)
    {
      ...
      std::vector<std::array<double, sizeof...(Is)>> result;
      result.reserve(bounds.size());
      for (auto const& current_bound : bounds) {
        std::array<double, sizeof...(Is)> tmp;
  ...
  Ts... names)
{
  ...
  std::vector<quad::Volume<double, n>> result;
