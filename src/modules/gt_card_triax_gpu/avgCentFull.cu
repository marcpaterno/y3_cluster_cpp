...
public:
  ...
  explicit avgCentCard(cosmosis::DataBlock& config);
...
avgCentCard::avgCentCard( DataBlock& cfg)
{
  auto rc = 
...
void
avgCentCard::set_sample(DataBlock& sample)
{
...
avgCentCard::operator()(double lo,
                        double lt,
                        double zt,
                        double lnM) const
{
...
std::vector<avgCentCard::volume_t>
avgCentCard::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgCentCard::module_label(), "lo", "lt", "zt", "lnm");
}

avgCentCard::grid_t
avgCentCard::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgCentCard)
