...
// y1_analysis_mor is a class that models the concept of "CosmoSISIntegrand",
...
public:
  ...
  explicit y1_analysis_mor(cosmosis::DataBlock& config);
...
y1_analysis_mor::y1_analysis_mor(DataBlock& config)
  : lc_lt()
  , mor()
  , omega_z()
  , dv_do_dz()
...
void
y1_analysis_mor::set_sample(DataBlock& sample)
{
...
std::vector<double>
y1_analysis_mor::operator()(double lo, double zt, double lnM) const
{
...
//
void
y1_analysis_mor::finalize_sample(
  cosmosis::DataBlock& sample,
...
char const*
y1_analysis_mor::module_label()
{
  return "y1_analysis_mor";
}
...
std::vector<y1_analysis_mor::volume_t>
y1_analysis_mor::make_integration_volumes(cosmosis::DataBlock& cfg)
{
  try {
...
DEFINE_COSMOSIS_VECTOR_INTEGRATION_MODULE(y1_analysis_mor)
