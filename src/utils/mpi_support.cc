...
namespace y3_cluster {
  ...
  get_mpi_info()
  {
    ...
    int rc = MPI_Comm_split_type(
      MPI_COMM_WORLD, MPI_COMM_TYPE_SHARED, result.global_rank, info, &local);
    if (rc != MPI_SUCCESS)
  ...
  std::ostream&
  operator<<(std::ostream& os, mpi_info const& info)
  {
  ...
  void
  record_timestamp(std::ostream& os, char const* label)
  {
    os << label << '\t' << std::setprecision(17) << MPI_Wtime()
