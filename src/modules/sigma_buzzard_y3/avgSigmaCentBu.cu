...
public:
  ...
  explicit avgSigmaCentBu(cosmosis::DataBlock& config);
...
avgSigmaCentBu::avgSigmaCentBu( DataBlock& cfg)
{
  auto rc = 
...
void
avgSigmaCentBu::set_sample(DataBlock& sample)
{
...
avgSigmaCentBu::operator()(   double lo,
                              double zt,
                              double lnM) const
{
...
std::vector<avgSigmaCentBu::volume_t>
avgSigmaCentBu::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgSigmaCentBu::module_label(), "lo", "zt", "lnm");
}

avgSigmaCentBu::grid_t
avgSigmaCentBu::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgSigmaCentBu)
