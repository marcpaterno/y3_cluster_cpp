...
def execute(block, config):
        ...
        basepath = "%s" % os.environ["Y3_CLUSTER_CPP_DIR"]
        dat=np.genfromtxt('%s/data/deltasigma_test/xi_nfwonly.txt' % basepath, delimiter=',')
        test_r=dat[:, 0]; test_test=dat[:, 1]
        test_interp=interp2d(Radii, M, Xi_1, kind='cubic')
        test_values=test_interp(test_r, test_mass)
        with open('%s/validation-data/deltasigma_xi_nfwonly.out' % basepath, 'w') as outf:
            outf.write('r\t ytrue\t ytest\n')
            for ii in range(len(test_r)):
                outf.write('%f\t%.12e\t%.12e\n'%(test_r[ii], test_test[ii], test_values[ii]))

        dat=np.genfromtxt('%s/data/deltasigma_test/Sigma_nfwonly.txt' % basepath, delimiter=',')
        test_r=dat[:, 0]; test_test=dat[:, 1]
        test_interp=interp2d(RDS, M, S1, kind='cubic')
        test_values=test_interp(test_r, test_mass)
        with open('%s/validation-data/deltasigma_Sigma_nfwonly.out' % basepath, 'w') as outf:
            outf.write('r\t ytrue\t ytest\n')
            for ii in range(len(test_r)):
                outf.write('%f\t%.12e\t%.12e\n'%(test_r[ii], test_test[ii], test_values[ii]))


        dat=np.genfromtxt('%s/data/deltasigma_test/xi_mm_2halo.txt' % basepath, delimiter=',')
        test_r=dat[:, 0]; test_test=dat[:, 1]
        ...
        with open('%s/validation-data/deltasigma_xi_mm_2halo.out' % basepath, 'w') as outf:
            outf.write('r\t ytrue\t ytest\n')
            for ii in range(len(test_r)):
                outf.write('%f\t%.12e\t%.12e\n'%(test_r[ii], test_test[ii], test_values[ii]))

        dat=np.genfromtxt('%s/data/deltasigma_test/Sigma_mm_2halo.txt' % basepath, delimiter=',')
        test_r=dat[:, 0]; test_test=dat[:, 1]
        test_interp=interp2d(RDS, zz, S2, kind='quintic')
        test_values=test_interp(test_r, test_z)
        with open('%s/validation-data/deltasigma_Sigma_mm_2halo.out' % basepath, 'w') as outf:
            outf.write('r\t ytrue\t ytest\n')
            for ii in range(len(test_r)):
                outf.write('%f\t%.12e\t%.12e\n'%(test_r[ii], test_test[ii], test_values[ii]))

        dat=np.genfromtxt('%s/data/deltasigma_test/bias.txt' % basepath, delimiter=',')
        test_mass=dat[:, 0]; test_test=dat[:, 2]
        test_z=dat[:, 1];
        test_interp=interp2d(zz, M, Bias, kind='cubic')
        test_value=test_interp(test_z, test_mass)
        with open('%s/validation-data/deltasigma_bias.out' % basepath, 'w') as outf:
