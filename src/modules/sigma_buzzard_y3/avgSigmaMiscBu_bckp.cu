...
private:
  ...
  // and the sigma_mis profile
  std::optional<y3_cuda::KAPPA_MAX> sigma_mis;
                              ...
                              double rmis) const
{
  ...
  double const sigma_v = (*sigma_mis)(scaled_Rmis, lnM, zt);
  auto const val = (sigma_v) * roffset_v *
  ...
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgSigmaMiscBu::module_label(), "lo", "zt", "lnm", "theta", "rmis");
}
