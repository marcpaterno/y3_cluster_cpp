...
TEST_CASE("Check that different ways to integrate bessel functions are equivalent")
{
    ...
    y3_cluster::Interp1D dcom(zz, dm_arr),
                         inverse_dcom(dm_arr, zz);

    std::array<y3_cluster::IntegrationRange, 4> zirs{{{0.1, 0.3}, {0.3, 0.5}, {0.5, 0.7}, {0.7, 0.9}}};

    cubacpp::QAG qag(0, 0, GSL_INTEG_GAUSS61, 20);
    for (const auto zir : zirs) {
        const double zstart = zir.transform(0.0),
                     zend = zir.transform(1.0),
                     rstart = dcom.eval(zstart) * h,
                     rend = dcom.eval(zend) * h;

        const auto r_normalization = (1.0 / 3.0) * (rend*rend*rend - rstart*rstart*rstart);

        const auto z_normalization = qag.with_range(zstart, zend)
                                        .integrate([&](double z) {
                                                return dvdodz(z);
                                                },
                                                1e-5, 1e-18);
        ...
        for (auto l = 0u; l < 10; l++) {
            for (auto ki = 0u; ki < 10; ki++) {
                ...
                CHECK(z_bessel_integral.value == Approx(r_bessel_integral.value).epsilon(2e-3));
