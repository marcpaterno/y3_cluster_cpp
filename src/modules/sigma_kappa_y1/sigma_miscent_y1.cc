...
// sigma_miscent_y1 is a class that models the concept of
// "ComoSISVectorIntegrand", and is thus suitable for use as the template
...
public:
  ...
  explicit sigma_miscent_y1(cosmosis::DataBlock& config);
...
sigma_miscent_y1::sigma_miscent_y1(DataBlock& config)
  : lc_lt()
  , mor()
  , omega_z()
  , dv_do_dz()
  ...
  , lo_lc()
  , sigma()
  , zo_low_(config.view<std::vector<double>>(sigma_miscent_y1::module_label(),
                                             "zo_low"))
  , zo_high_(config.view<std::vector<double>>(sigma_miscent_y1::module_label(),
                                              "zo_high"))
  , radii_(config.view<std::vector<double>>(sigma_miscent_y1::module_label(),
                                            "radii"))
{}

void
sigma_miscent_y1::set_sample(DataBlock& sample)
{
...
std::vector<double>
sigma_miscent_y1::operator()(double lo,
                             double lc,
                             double lt,
                             double zt,
                             double lnM,
...
//
void
sigma_miscent_y1::finalize_sample(
  cosmosis::DataBlock& sample,
...
char const*
sigma_miscent_y1::module_label()
{
  return "sigma_miscent_y1";
}

// The implementation of make_integration_volumes can be almost the same for
// any ComoSISVectorIntegrand-type class. Only the names and number of the
...
std::vector<sigma_miscent_y1::volume_t>
sigma_miscent_y1::make_integration_volumes(cosmosis::DataBlock& cfg)
{
  try {
