...
namespace y3_cluster {
    ...
    // ln(M200m) and z^true.
    //      <lam^true> = 1 + <lam^sat> 
    //      <lam^sat> = ((M200m - Mmin) / (M1 - Mmin))^alpha * ((1+z) / (1+zp))^epsilon
    //      Variance(lam^true) = <lam^sat> + (<lam^sat> * sigma_intr)^2
    //      <ln zeta> = ln Asz + Bsz * (lnM500c - lnMpsz) + Csz * (lnE(z) - lnE(zpsz))
    //      Variance(ln zeta) = sigma_lnzeta^2
    class MOR_Y3xSPT_t {
    public:
        MOR_Y3xSPT_t(double Mmin, double M1, double alpha, double epsilon,
            double sigintr, double Asz, double Bsz, double Csz, double siglnzeta,
            double r, double zplam, double lnMpsz, double zpsz, double omega_m,
            double omega_l, double omega_k, std::shared_ptr<Interp1D const> mass_conversion)
            : Mmin_(Mmin), M1_(M1), alpha_(alpha), epsilon_(epsilon)
            , sigintr_(sigintr), Asz_(Asz), Bsz_(Bsz), Csz_(Csz), siglnzeta_(siglnzeta)
            , r_(r), zplam_(zplam), lnMpsz_(lnMpsz), zpsz_(zpsz), omega_m_(omega_m)
            , omega_l_(omega_l), omega_k_(omega_k)
            , zetamrel_(Asz_, Bsz_, Csz_, lnMpsz_, zpsz_, omega_m_, omega_l_, omega_k_)
        ...
        double operator()(double lamtrue, double zeta, double ztrue, double lnM200m) {
            ...
            double const mean_lamsat = pow((std::exp(lnM200m) - Mmin_)/(M1_ - Mmin_), alpha_)
                                       * pow((1.0 + ztrue) / (1.0 + zplam_), epsilon_);
            double const mean_lamtrue = 1.0 + mean_lamsat;
            double const var_lam = mean_lamsat + mean_lamsat*mean_lamsat*sigintr_*sigintr_;
            ...
            double const one_over_det_cov = 1.0/(var_lnzeta * var_lam * (1.0 - r_*r_));

            double prefactor = 0.15915494309 * std::sqrt(one_over_det_cov) / zeta;
            double term1 = delta_lamtrue * delta_lamtrue * var_lnzeta;
            double term2 = -2.0*r_ * std::sqrt(var_lam*var_lnzeta) * delta_lamtrue * delta_lnzeta;
            double term3 = delta_lnzeta * delta_lnzeta * var_lam;

            return prefactor * std::exp( -0.5 * one_over_det_cov * (term1 + term2 + term3));
        }
