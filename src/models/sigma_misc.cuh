#ifndef Y3_CLUSTER_SIGMA_MISC_CUH
#define Y3_CLUSTER_SIGMA_MISC_CUH
...
namespace y3_cuda {
  ...
  public:
    SIGMA_MISC(y3_cuda::NFW_SIGMA_MIS const& nfw_sigma_mis,
              quad::Interp2D const& sigma2,
              quad::Interp2D const& bias,
              quad::Interp2D const& sigma_crit_inv,
              double tau)
      : _nfw_sigma_mis(nfw_sigma_mis), _sigma2(sigma2), _bias(bias), _sigma_crit_inv(sigma_crit_inv), _tau(tau)
    {}
    ...
    /*r in h^-1 Mpc */ /* M in h^-1 M_solar, represents M_{200} */
    {
      ...
      double const sig_mis = _nfw_sigma_mis(r, _tau, lnM);
