...
private:
  ...
  std::optional<y3_cuda::DSIGMA_FULL> gamma;
...
public:
  ...
  explicit avgGammaFullBu(cosmosis::DataBlock& config);
...
avgGammaFullBu::avgGammaFullBu( DataBlock& cfg)
{
  auto rc = 
...
void
avgGammaFullBu::set_sample(DataBlock& sample)
{
...
avgGammaFullBu::operator()(  double lo,
                              double zt,
                              double lnM) const
{
...
std::vector<avgGammaFullBu::volume_t>
avgGammaFullBu::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgGammaFullBu::module_label(), "lo", "zt", "lnm");
}

avgGammaFullBu::grid_t
avgGammaFullBu::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgGammaFullBu)
