...
#define CATCH_CONFIG_POSIX_SIGNALS
  ...
  !defined(CATCH_INTERNAL_CONFIG_NO_CPP17_UNCAUGHT_EXCEPTIONS) &&              \
...
namespace Catch {
  ...
  public: // construction/ assignment
    ...
    auto
    operator=(StringRef const& other) noexcept -> StringRef&
    {
      delete[] m_data;
      m_data = nullptr;
...
#pragma warning(push)
#pragma warning(                                                               \
  disable : 4180) // We attempt to stream a function (address) by const&, which
                  // MSVC complains about but is harmless
#endif
  ...
  namespace Matchers {
    ...
    namespace Vector {
      namespace Detail {
        ...
        size_t
        count(InputIterator first, InputIterator last, T const& item)
        {
          size_t cnt = 0;
        ...
        template <typename InputIterator, typename T>
        bool
        contains(InputIterator first, InputIterator last, T const& item)
        {
          for (; first != last; ++first) {
            if (*first == item) {
              return true;
      ...
      struct ContainsElementMatcher : MatcherBase<std::vector<T>> {
        ...
        bool
        match(std::vector<T> const& v) const override
        {
          for (auto const& el : v) {
            if (el == m_comparator) {
              return true;
      ...
      struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>> {
        ...
        {}
        bool
        match(std::vector<T> const& vec) const override
        {
    ...
    Contains(std::vector<T> const& comparator)
    {
    ...
    Equals(std::vector<T> const& comparator)
    {
    ...
    namespace Impl {
      namespace NSStringMatchers {
        ...
        struct StringHolder : MatcherBase<NSString*> {
          ...
          bool
          match(NSString* arg) const override
          {
            return false;
          }
...
namespace Catch {
  ...
  auto
  BenchmarkLooper::getResolution() -> uint64_t
  {
  ...
  auto
  BenchmarkLooper::needsMoreIterations() -> bool
  {
...
namespace Catch {
  ...
  // There is another overload, in catch_assertinhandler.h/.cpp, that only takes
...
namespace Catch {
  ...
  bool
  SourceLineInfo::empty() const noexcept
  {
    return file[0] == '\0';
  }
  bool
...
#define CATCH_PREPARE_EXCEPTION(type, msg)                                     \
  type(static_cast<std::ostringstream&&>(Catch::ReusableStringStream().get()   \
                                         << msg)                               \
         .str())
...
namespace Catch {
  ...
  int
  Config::benchmarkResolutionMultiple() const
  {
...
#if (defined(CATCH_PLATFORM_WINDOWS) && defined(CATCH_CONFIG_WINDOWS_SEH)) ||  \
  defined(CATCH_CONFIG_POSIX_SIGNALS)
namespace {
...
namespace Catch {
  ...
  LONG CALLBACK
  FatalConditionHandler::handleVectoredException(
...
namespace Catch {
  ...
  void
  FatalConditionHandler::handleSignal(int sig)
  {
...
namespace Catch {
  ...
  void
  addReporter(IStreamingReporterPtr& existingReporter,
              IStreamingReporterPtr&& additionalReporter)
  {
...
namespace {
  ...
  bool
  almostEqualUlps(FP lhs, FP rhs, int maxUlpDiff)
  {
...
namespace Catch {
  namespace Matchers {
    ...
    WithinULP(double target, int maxUlpDiff)
    {
    ...
    Floating::WithinUlpsMatcher
    WithinULP(float target, int maxUlpDiff)
    {
      return Floating::WithinUlpsMatcher(
...
namespace Catch {
  ...
  RandomNumberGenerator::operator()(result_type n) const
  {
    return std::rand() % n;
  }
  RandomNumberGenerator::result_type
  RandomNumberGenerator::operator()() const
  {
    return std::rand() % (max)();
  }
}
...
namespace Catch {
  ...
  namespace {
    ...
    public: // IRegistryHub
      ...
      getExceptionTranslatorRegistry() override
      {
...
#undef CATCH_CONFIG_DEFAULT_REPORTER
    ...
    Catch::Totals
    runTests(std::shared_ptr<Config> const& config)
    {
  ...
  int
  Session::applyCommandLine(int argc, char* argv[])
  {
  ...
  int
  Session::run(int argc, char* argv[])
  {
    if (m_startupExceptions)
      return 1;
...
#if defined(WIN32) && defined(UNICODE)
  int
  Session::run(int argc, wchar_t* const argv[])
  {
...
namespace Catch {
  ...
  void
  StringRef::swap(StringRef& other) noexcept
  {
  ...
  auto
  StringRef::isOwned() const noexcept -> bool
  {
    return m_data != nullptr;
  }
  auto
  StringRef::isSubstring() const noexcept -> bool
  {
    return m_start[m_size] != '\0';
  }

  void
  StringRef::takeOwnership()
  {
    if (!isOwned()) {
  ...
  auto
  StringRef::numberOfCharacters() const noexcept -> size_type
  {
  ...
  auto
  operator+(StringRef const& lhs, StringRef const& rhs) -> std::string
  {
    std::string str;
  ...
  auto
  operator+(StringRef const& lhs, const char* rhs) -> std::string
  {
    return std::string(lhs) + std::string(rhs);
  }
  auto
  operator+(char const* lhs, StringRef const& rhs) -> std::string
  {
    return std::string(lhs) + std::string(rhs);
  }
...
namespace Catch {
  ...
  bool
  matchTest(TestCase const& testCase,
            TestSpec const& testSpec,
            IConfig const& config)
  {
...
namespace Catch {
  namespace TestCaseTracking {
    ...
    {}
    bool
    TrackerBase::TrackerHasName::operator()(ITrackerPtr const& tracker) const
    {
    ...
    TrackerBase::nameAndLocation() const
    {
    ...
    {
      auto it = std::find_if(
        m_children.begin(), m_children.end(), TrackerHasName(nameAndLocation));
    ...
    bool
    TrackerBase::isIndexTracker() const
    {
      return false;
    }
    ...
    bool
    IndexTracker::isIndexTracker() const
    {
      return true;
    }

    IndexTracker&
    IndexTracker::acquire(TrackerContext& ctx,
                          NameAndLocation const& nameAndLocation,
                          int size)
    {
    ...
    int
    IndexTracker::index() const
    {
      return m_index;
    }

    void
    IndexTracker::moveNext()
    {
      m_index++;
      m_children.clear();
    }
...
namespace Catch {
  ...
  void
  TestSpecParser::startNewMode(Mode mode, std::size_t start)
  {
    m_mode = mode;
    m_start = start;
  }
  ...
  std::string
  TestSpecParser::subString() const
  {
...
namespace Catch {
  ...
  std::string
  StringMaker<char>::convert(char value)
  {
    if (value == '\r') {
      return "'\\r'";
    } else if (value == '\f') {
  ...
  std::string
  StringMaker<signed char>::convert(signed char c)
  {
...
namespace Catch {
  ...
  std::string
  WildcardPattern::adjustCase(std::string const& str) const
  {
    return m_caseSensitivity == CaseSensitive::No ? toLower(str) : str;
  }
}
...
namespace Catch {
  ...
  XmlWriter::ScopedElement
  XmlWriter::scopedElement(std::string const& name)
  {
  ...
  XmlWriter&
  XmlWriter::writeText(std::string const& text, bool indent)
  {
    if (!text.empty()) {
...
namespace Catch {
  ...
  // required Alternatively we could use stringstream, but its performance is...
  // not good.
  std::string
...
namespace Catch {
  ...
  ReporterPreferences
  CompactReporter::getPreferences() const
  {
...
namespace Catch {
  ...
  void
  MultipleReporters::add(IStreamingReporterPtr&& reporter)
  {
    m_reporters.push_back(std::move(reporter));
  }

  ReporterPreferences
  MultipleReporters::getPreferences() const
  {
    return m_reporters[0]->getPreferences();
  }

  std::set<Verbosity>
  MultipleReporters::getSupportedVerbosities()
  {
    return std::set<Verbosity>{};
  }

  void
  MultipleReporters::noMatchingTestCases(std::string const& spec)
  {
  ...
  void
  MultipleReporters::testRunStarting(TestRunInfo const& testRunInfo)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::testGroupStarting(GroupInfo const& groupInfo)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::testCaseStarting(TestCaseInfo const& testInfo)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::sectionStarting(SectionInfo const& sectionInfo)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::sectionEnded(SectionStats const& sectionStats)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::testRunEnded(TestRunStats const& testRunStats)
  {
    for (auto const& reporter : m_reporters)
  ...
  void
  MultipleReporters::skipTest(TestCaseInfo const& testInfo)
  {
    for (auto const& reporter : m_reporters)
  ...
  bool
  MultipleReporters::isMulti() const
  {
    return true;
  }
