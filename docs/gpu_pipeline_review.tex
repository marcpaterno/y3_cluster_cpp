...
\texttt{shearTotSimple} & \textcolor{green!50!black}{\textbf{TESTED}} & Total $\gamma_t$ via haloModel + sigmaCritInv \\
\texttt{sigma1hSimple} & Built & Ready to test (1-halo only) \\
\texttt{bSelMargGPU} & \textcolor{blue}{\textbf{NEW}} & Unified $(P_1, I_1, J)$ operators, 3D + GL$_\theta$ \\
\bottomrule
\end{tabular}
...
\subsection{Test Results: \texttt{shearTotSimple}}

Successfully tested on HPC (2026-05-25). Computes $N \times \langle\gamma_t(R)\rangle$ via 3D integration over $(l_o, z_t, \ln M)$.
...
$z$ Range & $R=0.5$ Mpc/h & $R=1.0$ Mpc/h & $R=2.0$ Mpc/h \\
\midrule
$[0.20, 0.35]$ & 61.3 & 34.0 & 15.2 \\
$[0.35, 0.50]$ & 92.8 & 52.6 & 23.2 \\
$[0.50, 0.65]$ & 93.3 & 53.0 & 22.2 \\
\bottomrule
\end{tabular}
\caption{GPU $N \times \langle\gamma_t(R)\rangle$ results (dimensionless shear $\times$ counts).}
\end{table}

\paragraph{Note:} Shear profile decreases with radius as expected ($\gamma_t \propto R^{-1}$ approximately). Uses \texttt{GAMMA\_MAX} model which reads directly from \texttt{haloModel} section (no adapter needed).

\paragraph{Timing:} Module execution took 0.31 seconds.
...
\begin{itemize}
  ...
  \item \filepath{src/models/p\_operator\_gpu\_t.cuh} -- Costanzi-2026 $P[X]$ operators (GPU)
  \item \filepath{src/modules/simple\_gpu/numberCountsFull.cu} -- 4D integration
  \item \filepath{src/modules/simple\_gpu/bSelMargGPU.cu} -- Unified $(P_1, I_1, J)$ module
  ...
  \item \filepath{cosmosis\_tests/gpu/shear\_tot\_simple\_test.ini} -- Shear test config
  \item \filepath{cosmosis\_tests/gpu/b\_sel\_marg\_test.ini} -- $P_1, I_1, J$ test config
...
\begin{itemize}
  ...
  \item[\textcolor{green!50!black}{$\checkmark$}] \texttt{halomodel\_adapter.py} created for sigmaTotSimple
  \item[\textcolor{green!50!black}{$\checkmark$}] \texttt{sigmaTotSimple} tested successfully
  \item[\textcolor{green!50!black}{$\checkmark$}] \texttt{shearTotSimple} tested successfully
\end{itemize}

\subsection{Immediate Next Steps}

\paragraph{1. Known bug (team review needed):} Both \texttt{sig\_max.cuh:67} and \texttt{gamma\_max.cuh:63} use \texttt{std::max(sig\_1, sig\_2)} instead of \texttt{sig\_1 + sig\_2}. This returns $\max(\Sigma_{1h}, b\Sigma_{2h})$ instead of the correct sum.

\paragraph{2. Run full 12-bin test:}
Configure \texttt{numberCountsFull} with all DES Y3 bins (4 richness $\times$ 3 redshift).

\paragraph{3. Test \texttt{sigma1hSimple}:} 1-halo only surface density (optional).
...
Phase 4: Test sigma/shear modules & 1--2 days & Medium & \textcolor{green!50!black}{\textbf{DONE}} \\
...
\begin{itemize}
  ...
  \item[\textcolor{green!50!black}{$\checkmark$}] \texttt{shearTotSimple} tested with haloModel + sigmaCritInv
  \item[\textcolor{orange}{$\circ$}] \texttt{sigma1hSimple} ready to test (1-halo only)
...
\begin{enumerate}
  \item \textbf{Team review}: Address \texttt{std::max} bug in \texttt{sig\_max.cuh} and \texttt{gamma\_max.cuh}
  \item Run full 12-bin \texttt{numberCountsFull} test (4 richness $\times$ 3 redshift)
  \item \textbf{Phase 5}: Cross-validate GPU results against CPU \texttt{sel\_function} pipeline
  \item Optionally test \texttt{sigma1hSimple} (1-halo only version)
\end{enumerate}
