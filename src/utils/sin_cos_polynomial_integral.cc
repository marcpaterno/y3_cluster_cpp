...
sinusoid_polynomial_iteration(const std::pair<double, double> curstep,
                              const int n,
                              const double x,
                              const double cosx,
                              const double sinx)
...
y3_cluster::sinusoid_polynomial_taylor_approx(int n,
                                              const double range_min,
                                              const double range_max)
{
  // n + 1 power because we are integrating
  double powmin = integer_pow(range_min, n + 1),
         powmax = integer_pow(range_max, n + 1), sinint_min_sum = 0,
         cosint_min_sum = 0, sinint_max_sum = 0, cosint_max_sum = 0;
...
std::pair<double, double>
y3_cluster::sinusoid_polynomial_integral(int n,
                                         const double range_min,
                                         const double range_max)
{
  if ((std::abs(range_max - range_min) < 1) && (range_max < 2) &&
      (range_min > -2)) {
    return sinusoid_polynomial_taylor_approx(n, range_min, range_max);
  }

  const double sin_min = std::sin(range_min), sin_max = std::sin(range_max),
               cos_min = std::cos(range_min), cos_max = std::cos(range_max);
  ...
  if (n >= 0) {
    for (auto i = 1; i <= n; i++) {
      vals_min =
        sinusoid_polynomial_iteration(vals_min, i, range_min, cos_min, sin_min);
      vals_max =
        sinusoid_polynomial_iteration(vals_max, i, range_max, cos_max, sin_max);
    }
  } else if (n < 0) {
...
y3_cluster::sinusoid_polynomial_integrals(int minn,
                                          int maxn,
                                          const double range_min,
                                          const double range_max)
{
  ...
  std::pair<std::vector<double>, std::vector<double>> output = {
    std::vector<double>(diff + 1), std::vector<double>(diff + 1)};

  // On range around zero, defer to taylor expansion
  if ((std::abs(range_max - range_min) < 2) && (range_max < 2) &&
      (range_min > -2)) {
    for (auto i = 0; i < diff; i++) {
      const auto [sin, cos] =
        sinusoid_polynomial_taylor_approx(minn + i, range_min, range_max);
      output.first[i] = sin;
      output.second[i] = cos;
    }
    return output;
