...
// Obtain a std::vector<double> from a DataBlock, from section 'section', with
// parameter name 'name'. The parameter in question may actually have been
// stored as a vector<double>, or double, or vector<int>, or int. Both 'section'
// and 'name' must be non-null.
std::vector<double> get_vector_double(cosmosis::DataBlock& db,
                                      const char* section,
                                      const char* name);
std::vector<double> get_vector_double(cosmosis::DataBlock& db,
                                      std::string section,
              ...
              T default_)
{
  T a;
  auto retval = db.get_val<T>(section, value, a);
  if (retval != DBS_SUCCESS) { return default_; }
  return a;
}
...
get_datablock(cosmosis::DataBlock& db, const char* section, const char* value)
{
  ...
  if (retval != DBS_SUCCESS || typ != DBT_DOUBLEND) {
    if (typ != DBT_DOUBLEND) retval = DBS_WRONG_VALUE_TYPE;
    throw std::runtime_error(
      "Error (" + std::string(datablock_status_str(retval)) +
      ") when retrieving: " + std::string(section) + ":" + std::string(value));
  }
              ...
              cosmosis::ndarray<double> default_)
{
  datablock_type_t typ;
  auto retval = db.get_type(section, value, typ);
  if (retval != DBS_SUCCESS || typ != DBT_DOUBLEND) { return default_; }
  return db.view<cosmosis::ndarray<double>>(section, value);
}
