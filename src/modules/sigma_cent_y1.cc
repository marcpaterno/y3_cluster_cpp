...
sigma_cent_y1::sigma_cent_y1(DataBlock& config) : 
	lc_lt(),
        ...
        int_zo_zt(),
        sigma(),
	zo_low_(config.view<std::vector<double>>(sigma_cent_y1::module_label(), "zo_low")),
        zo_high_(config.view<std::vector<double>>(sigma_cent_y1::module_label(), "zo_high")), 
        radii_(config.view<std::vector<double>>(sigma_cent_y1::module_label(), "radii")){}
...
sigma_cent_y1::operator()(double lo, double lt, double zt, double lnM) const
{
  ...
  for (std::size_t i =0; i != zo_low_.size(); i++){
      ...
      for (std::size_t j =0; j != radii_.size(); j++){
        results[i*(radii_.size()+1)+j+1] = (*sigma)(radii_[j], lnM, zt) * val;
      }
