...
public:
  ...
  explicit avgWpCentBu(cosmosis::DataBlock& config);
...
avgWpCentBu::avgWpCentBu( DataBlock& cfg)
{
  auto rc = 
...
void
avgWpCentBu::set_sample(DataBlock& sample)
{
...
avgWpCentBu::operator()(double lo,
                      double zt,
                      double lnM) const
{
...
std::vector<avgWpCentBu::volume_t>
avgWpCentBu::make_integration_volumes(
  cosmosis::DataBlock& cfg)
{
  return y3_cuda::make_integration_volumes_wall_of_numbers(
    cfg, avgWpCentBu::module_label(), "lo", "zt", "lnm");
}

avgWpCentBu::grid_t
avgWpCentBu::make_grid_points(cosmosis::DataBlock& cfg)
{
...
DEFINE_COSMOSIS_CUDA_INTEGRATION_MODULE(avgWpCentBu)
