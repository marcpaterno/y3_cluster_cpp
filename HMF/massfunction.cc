...
int sigma2_at_M_arr(double M,  double*k, double*P, int Nk, double om, double*s2){
...
int dndM_sigma2_precomputed(double M, double*sigma2, double*sigma2_top, double*sigma2_bot, double Omega_m, double*dndM){
  ...
  for(i = 0; i < 1; i++){
    dndM[i] = dndMconst*Gsigma[i]*log(sigma2_top[i]/sigma2_bot[i])/(M*M);
  }
  free(sigma);
  free(Gsigma);
  return 0;
...
int dndM_at_M_arr(double M, double*k, double*P, int Nk, double om, double*dndM){
