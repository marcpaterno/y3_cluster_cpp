...
y3_cluster::SigmaModule::SigmaModule(cosmosis::DataBlock& config)
{
  ...
  ln_R2d_min =
    std::log(get_datablock<double>(config, OPTION_SECTION, "R_perp_min"));
  ln_R2d_max =
    std::log(get_datablock<double>(config, OPTION_SECTION, "R_perp_max"));
  R2d_bins = get_datablock<int>(config, OPTION_SECTION, "R_perp_bins");

  ln_R3d_min =
    std::log(get_datablock<double>(config, OPTION_SECTION, "Radii_min"));
  ln_R3d_max =
    std::log(get_datablock<double>(config, OPTION_SECTION, "Radii_max"));
...
y3_cluster::SigmaModule::execute(cosmosis::DataBlock& sample)
{
  ...
  sample.put_val<cosmosis::ndarray<double>>(
    "deltasigma", "Xi_2", {xi_2halo, extents_3d});
  ...
  sample.put_val<cosmosis::ndarray<double>>(
    "deltasigma", "Xi_1", {xi_1halo, extents_3d_1});
  ...
  sample.put_val<cosmosis::ndarray<double>>(
    "deltasigma", "sigma_1", {sigma_1halo, extents_2d_1});
  ...
  sample.put_val<cosmosis::ndarray<double>>(
    "deltasigma", "sigma_2", {sigma_2halo, extents_2d});
