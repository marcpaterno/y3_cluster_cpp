...
def setup(options):
    ...
    flag = options[section,"flag"]

    # Need to read in DS data and covariance here

    return cov, NCdata, Mdata, Om_corr, lnAS_corr, flag



def execute(block, config):

        covmat, ncdata, Mdata, Om_corr, lnAS_corr, flag = config

        nncent = block["mass2sigma", "NC_cent"]
        MMcent = block["mass2sigma", "mcent_h"]
        nntot = block["mass2sigma", "NC_tot"]
        MMtot = block["mass2sigma", "m_average_h"]
        if flag == 1:
           nn = nncent
           logM = np.log10(MMcent)
        elif flag == 2 or flag ==3:
           nn = nntot
           logM = np.log10(MMtot)
        else:
           raise Exception("flag not properly set, only values of 1, 2 or 3 are allowed")
        ...
        print("theory NC cent, NCtotal:", nncent, nntot)
        ...
        # need to incorporate an option with Deltasigma
        


        block[section_names.likelihoods, 'MassSigmaLike_like'] = loglike1
