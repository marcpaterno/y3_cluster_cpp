...
def execute(block, config):
    ...
    np.savetxt('Redges.txt', Redges)
    np.savetxt('aver_ds.txt', aver_ds)
    np.savetxt('model_ncs.txt', model_ncs)
...
def make_plots(ngriddim, nvoldim, rad_edges, model_vector, data_vector, data_err, ncs, model_ncs, nc_covmat):
    ...
    axs[1, 0].set_ylabel(r'$\Delta \Sigma$ $[M_\odot h/pc^2]$')
    axs[1, 2].legend(loc = 0)
    axs[0, 2].legend(loc = 0)
    plt.savefig('DS_demo.png')
    plt.show()
