...
// kappa_miscent_y1 is a class that models the concept of "CosmoSISVectorIntegrand",
...
private:
  ...
  std::optional<AVERAGE_SCI_t> sci;
  std::optional<ANGLE_TO_DIST_t> arc2dist;
...
kappa_miscent_y1::kappa_miscent_y1(DataBlock& config) : 
	...
	sci(),
	arc2dist(),
	zo_low_(config.view<std::vector<double>>(kappa_miscent_y1::module_label(), "zo_low")),
        zo_high_(config.view<std::vector<double>>(kappa_miscent_y1::module_label(), "zo_high")), 
        radii_(config.view<std::vector<double>>(kappa_miscent_y1::module_label(), "radii")){}
...
std::vector<double>
kappa_miscent_y1::operator()(double lo, double lc, double lt, double zt, double lnM, double rmis, double theta) const
{
  ...
  for (std::size_t i =0; i != zo_low_.size(); i++){
	...
	dist = (*arc2dist)(radii_[j], zt);
        scaled_Rmis=std::sqrt(dist*dist + rmis*rmis + 2*rmis*dist * std::cos(theta));
        results[i*(radii_.size()+1)+j+1] = (*sci)(zt)*(*sigma)(scaled_Rmis, lnM, zt) * val;
      }
...
//
void
kappa_miscent_y1::finalize_sample(
  cosmosis::DataBlock& sample,
  std::vector<integration_results_v> const& results) const
{
  ...
  sample.put_val(module_label(), "kappa_radius_arcmin", radii_);

  sample.put_val(module_label(), "totKappa_vals", totSigma_vals);
  sample.put_val(module_label(), "totKappa_errors", totSigma_errors);
  sample.put_val(module_label(), "totKappa_probs", totSigma_probs);
}
...
DEFINE_COSMOSIS_VECTOR_INTEGRATION_MODULE(kappa_miscent_y1);
