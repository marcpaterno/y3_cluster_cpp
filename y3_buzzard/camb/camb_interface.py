...
def get_choice(options, name, valid, default=None, prefix=""):
    ...
    if choice not in valid:
        raise ValueError(
            "Parameter setting '{}' in camb must be one of: {}.  You tried: {}".format(
                name, valid, choice
            )
        )
...
def setup(options):
    ...
    more_config["max_printed_errors"] = options.get_int(
        opt, "max_printed_errors", default=20
    )
    ...
    if "theta_H0_range" in more_config["cosmology_params"]:
        more_config["cosmology_params"] = [
            float(x) for x in more_config["cosmology_params"]["theta_H0_range"].split()
        ]

    more_config["do_reionization"] = options.get_bool(
        opt, "do_reionization", default=True
    )
    more_config["use_optical_depth"] = options.get_bool(
        opt, "use_optical_depth", default=True
    )
    ...
    more_config["use_tabulated_w"] = options.get_bool(
        opt, "use_tabulated_w", default=False
    )
    ...
    if halofit_version not in known_halofit_versions:
        raise ValueError(
            "halofit_version must be one of : {}.  You put: {}".format(
                known_halofit_versions, halofit_version
            )
        )
    ...
    more_config["zmin_background"] = options.get_double(
        opt, "zmin_background", default=more_config["zmin"]
    )
    more_config["zmax_background"] = options.get_double(
        opt, "zmax_background", default=more_config["zmax"]
    )
    more_config["nz_background"] = options.get_int(
        opt, "nz_background", default=more_config["nz"]
    )

    more_config["transfer_params"] = get_optional_params(
        options, opt, ["k_per_logint", "accurate_massive_neutrino_transfers"]
    )
    # Adjust CAMB defaults
    more_config["transfer_params"]["kmax"] = options.get_double(
        opt, "kmax", default=10.0
    )
    ...
    more_config["kmax"] = options.get_double(
        opt, "kmax", more_config["transfer_params"]["kmax"]
    )
    more_config["kmax_extrapolate"] = options.get_double(
        opt, "kmax_extrapolate", default=more_config["kmax"]
    )
    more_config["nk"] = options.get_int(opt, "nk", default=200)

    more_config["power_spectra"] = options.get_string(
        opt, "power_spectra", default="delta_tot"
    ).split()
    bad_power = []
    ...
    if bad_power:
        ...
        raise ValueError(
            """These matter power types are not known: {}.
Please use any these (separated by spaces): {}""".format(
                bad_power, good_power
            )
        )
...
def extract_recombination_params(block, config, more_config):
    ...
    min_a_evolve_Tm = block.get_double(
        "recfast", "min_a_evolve_Tm", default=default_recomb.min_a_evolve_Tm
    )
    RECFAST_fudge = block.get_double(
        "recfast", "RECFAST_fudge", default=default_recomb.RECFAST_fudge
    )
    RECFAST_fudge_He = block.get_double(
        "recfast", "RECFAST_fudge_He", default=default_recomb.RECFAST_fudge_He
    )
    RECFAST_Heswitch = block.get_int(
        "recfast", "RECFAST_Heswitch", default=default_recomb.RECFAST_Heswitch
    )
    RECFAST_Hswitch = block.get_bool(
        "recfast", "RECFAST_Hswitch", default=default_recomb.RECFAST_Hswitch
    )
...
def extract_reionization_params(block, config, more_config):
    ...
    if more_config["do_reionization"]:
        if more_config["use_optical_depth"]:
            tau = block[cosmo, "tau"]
            reion = camb.reionization.TanhReionization(
                use_optical_depth=True, optical_depth=tau
            )
        else:
            sec = "reionization"
            redshift = block[sec, "redshift"]
            delta_redshift = block[sec, "delta_redshift"]
            reion_params = get_optional_params(
                block,
                sec,
                [
                    "fraction",
...
def extract_nonlinear_params(block, config, more_config):
    ...
    return camb.nonlinear.Halofit(**more_config["nonlinear_params"], **hmcode_params)
...
def extract_camb_params(block, config, more_config):
    ...
    cosmology_params = get_optional_params(
        block,
        cosmo,
        [
            "TCMB",
    ...
    if block.has_value(cosmo, "massless_nu"):
        warnings.warn(
            "Parameter massless_nu is being ignored. Set nnu, the effective number of relativistic species in the early Universe."
        )

    if (
        block.has_value(cosmo, "omega_nu") or block.has_value(cosmo, "omnuh2")
    ) and not (block.has_value(cosmo, "mnu")):
        warnings.warn(
            "Parameter omega_nu and omnuh2 are being ignored. Set mnu and num_massive_neutrinos instead."
        )
    ...
    if (
        np.isclose(p.omnuh2, 0)
        and "nnu" in cosmology_params
        and not np.isclose(cosmology_params["nnu"], p.num_nu_massless)
    ):
    ...
    if want_perturbations:
            ...
            z = np.concatenate(
                (
                    ...
                    np.linspace(
                        more_config["zmid"],
                        more_config["zmax"],
                        more_config["nz"] - more_config["nz_mid"],
                    ),
                )
            )[::-1]
        else:
        ...
        p.set_matter_power(
            redshifts=z,
            nonlinear=config["NonLinear"] in ["NonLinear_both", "NonLinear_pk"],
            **more_config["transfer_params"],
        )
...
def save_derived_parameters(r, p, block):
    ...
    for cosmosis_name, CAMB_name, scaling in [
        ("h0", "h", 1),
        ("hubble", "h", 100),
        ("omnuh2", "omnuh2", 1),
        ("n_eff", "N_eff", 1),
        ...
        if block.has_value(names.cosmological_parameters, cosmosis_name):
            ...
            if not np.isclose(input_value, CAMB_value, rtol=0.002):
                warnings.warn(
                    f"Parameter {cosmosis_name} inconsistent: input was {input_value} but value is now {CAMB_value}."
                )
...
def compute_growth_rates(r, block, P_tot, k, z, more_config):
    if P_tot is None:
        ...
        P_tot = r.get_matter_power_interpolator(
            nonlinear=False, extrap_kmax=more_config["kmax_extrapolate"]
        )
...
def save_matter_power(r, p, block, more_config):
    ...
    k = np.logspace(
        np.log10(more_config["kmin"]), np.log10(kmax_power), more_config["nk"]
    )
