...
using y3_cluster::sinusoid_polynomial_integral,
  y3_cluster::sinusoid_polynomial_integrals, y3_cluster::integer_pow;

TEST_CASE("Test analytic sin/cos polynomial integrals")
{
  cubacpp::QAG qag;

  std::vector<std::pair<double, double>> ranges{{{-7, -3},
                                                 {-3, -2},
                                                 {-2, -1},
                                                 {-0.3, -0.1},
                                                 {0.1, 2},
  ...
  for (auto const& [minx, maxx] : ranges) {
    ...
    for (auto pow = minpow; pow < maxpow; pow++) {
      ...
      CHECK(sin_integral.value ==
            Approx(sin_results[pow - minpow]).epsilon(1e-5));
      CHECK(cos_integral.value ==
            Approx(cos_results[pow - minpow]).epsilon(1e-5));
    }
  }

  std::vector<std::pair<double, double>> small_ranges{{{-2.6, -2.5},
                                                       {-2.5, -2},
                                                       {-2, -0.01},
                                                       {-1, -0.1},
                                                       {-0.3, -0.01},
  ...
  for (auto const& [minx, maxx] : small_ranges) {
    for (auto pow = minpow; pow < maxpow; pow++) {
      ...
      const auto [sin_result, cos_result] =
        y3_cluster::sinusoid_polynomial_taylor_approx(pow, minx, maxx);
