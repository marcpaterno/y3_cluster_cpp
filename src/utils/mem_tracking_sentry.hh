...
namespace y3_cluster {
  ...
  public:
    explicit MemTrackSentry(std::ostream* os,
                            ...
                            int grid_point_id = -1)
      : os_(os)
      , module_label_(module_label)
      , label_(label)
      , volume_id_(volume_id)
      , grid_point_id_(grid_point_id)
      , integrand_dev_mem_footprint(object_mem_size)
    {
      print_free_device_mem("start");
    }

    void
    print_free_device_mem(std::string user_string)
    {
      if (os_) {
        ...
        *os_ << free_physmem << "," << integrand_dev_mem_footprint << ","
             << module_label_ << "," << label_ << "," << user_string << ","
             << volume_id_ << "," << grid_point_id_ << std::endl;
      }
    }

    ~MemTrackSentry() { print_free_device_mem("end"); }
  };
}
